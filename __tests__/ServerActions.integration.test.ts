...
const server = createServer((req, res) => {
  ...
  } else if (req.url === '/api/getAllStudentsByTeacher' && req.method === 'GET') {
...
describe('ServerActions Integration Tests', () => {
  ...
  describe('studentStats', () => {
    it('should return student answers if userId is provided', async () => {
      ...
      const response = await request(server).get(`/api/studentStats?userId=${mockUserId}`);
  ...
  describe('getAllStudentsByTeacher', () => {
    it('should return all students and their combined answers for a teacher', async () => {
      ...
      const response = await request(server).get('/api/getAllStudentsByTeacher');

      expect(response.status).toBe(200);
      expect(response.body.allStudents).toEqual(mockTeacherWithStudents.students);
